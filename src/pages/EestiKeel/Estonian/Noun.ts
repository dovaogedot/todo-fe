...
export class Noun extends Word {
  ...
  agree(query: Case | Number): string {
    ...
    const n = (query & (Number.Singular | Number.Plural)) || Number.Singular
    const oma = n == Number.Singular ? this.#cases.oma : this.#cases.omaPl

    switch (c) {
      case Case.Nimetav:
        return n == Number.Singular ? this.#cases.nime : this.#cases.oma + 'd'
      case Case.Omastav:
        return n == Number.Singular ? this.#cases.oma : this.#cases.omaPl
      case Case.Osastav:
        return n == Number.Singular ? this.#cases.osa : this.#cases.osaPl
